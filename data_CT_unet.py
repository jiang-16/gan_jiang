...
class CTProjectionDataset(Dataset):
    def __init__(self, projection_dir, image_dir):
        ...
        self.images = [f.replace('.mat', '.mat') for f in self.projections]
    ...
    def __getitem__(self, idx):
        ...
        projection = mat_data['proj2']  # 替换'whateverKeyYouHave'为.mat文件中实际的键
        ...
        image = sio.loadmat(image_file_path)
        image = image['proj1']  # 确保为灰度图
        ...
        # 按照 (x - min) / (max - min) 的公式进行归一化到 [0, 1]
        img_np_normalized = (img_np_clipped - min_val) / (max_val - min_val)
