...
optimizer = optim.Adam(list(resnet.parameters()), lr=0.0001)
num_epochs = 500
...
# 按照 (x - min) / (max - min) 的公式进行归一化到 [0, 1]
            ...
            plt.imsave(os.path.join('./epoch/resnet/', f'epoch_{epoch + 1}_batch_{batch_idx + 1}.png'),combined_image)
...
# 计算平均损失
    ...
    if (epoch + 1) % 100 == 0:
        torch.save(resnet.state_dict(), os.path.join('./Pretrained/', f'resnet_params_epoch_{epoch + 1}.pth'))
torch.save(resnet.state_dict(), './Pretrained/resnet_params.pth')